...
export function arrayPackBasicNonFloat(format: string, v: string | number) {
    it('ignores whitespace in the format string', () => {
        const subject = makePack(format + ' \t\n\v\f\r' + format)
...
export function arrayPack32BitBe(_format: string) {
    ...
    it('throws when given a Float', () => {
        expect(() => {
            makePack(format())([0.2])
        }).toThrow('Cannot pack value "0.2" as integer')
...
export function arrayPack16BitBe(_format: string) {
    ...
    it('throws when given a float instead of integer', () => {
        expect(() => {
            makePack(format())([0.2])
        }).toThrow('Cannot pack value "0.2" as integer.')
    })
...
export function makePack(format: string): Packer['pack'] {
    const pack = new Packer(format)
